...
function CatCardSwiper(props){
  ...
  var catWeights=(weights[cat.id]||[]).slice().reverse();
...
function CatsPage(props){
  ...
  var catWeights=(weights[selId]||[]).slice().reverse();
  var catHealths=(healths[selId]||[]).slice().reverse();
...
function CalendarPage(props){
  ...
  var [selDate,setSelDate]=useState(today.toISOString().slice(0,10));
  var [showForm,setShowForm]=useState(false);
  var [editRec,setEditRec]=useState(null);
  var [form,setForm]=useState({date:today.toISOString().slice(0,10),type:"換貓砂",note:"",catIds:[],photos:[]});
  ...
  var selRecs=records.filter(function(r){return r.date===selDate;});
  ...
  function openAdd(){setEditRec(null);setForm({date:selDate,type:"換貓砂",note:"",catIds:[],photos:[]});setShowForm(true);onModalOpen();}
  function openEdit(rec){setEditRec(rec);setForm(Object.assign({catIds:[],photos:[]},rec));setShowForm(true);onModalOpen();}
  function handleCalPhoto(e){var files=Array.prototype.slice.call(e.target.files);var current=(form.photos||[]).slice();var count=0;function readNext(){if(count>=files.length)return;var reader=new FileReader();var file=files[count];count++;reader.onload=function(ev){current=current.concat([ev.target.result]);setForm(function(prev){return Object.assign({},prev,{photos:current.slice()});});readNext();};reader.readAsDataURL(file);}readNext();e.target.value="";}
  function handleSave(){console.log('handleSave called, form:', form);var ft=Object.assign({},form);if(!ft.date)ft.date=new Date().toISOString().slice(0,10);var next=editRec?records.map(function(r){return r.id===editRec.id?Object.assign({},ft,{id:editRec.id}):r;}):records.concat([Object.assign({},ft,{id:Date.now()+''})]);console.log('saving records:', next.length);saveRecords(next);if(!editRec){var newRec=next[next.length-1];setDetailRec(newRec);}setShowForm(false);onModalClose();}
  function handleDelete(id){saveRecords(records.filter(function(r){return r.id!==id;}));}
  function typeLabel(t){if(t==="換貓砂")return "💨 換貓砂";if(t==="洗貓砂盆")return "🚿 洗貓砂盆";if(t==="洗玩具用品")return "🧸 洗玩具用品";if(t==="洗澡")return "🛁 洗澡";return "🌀 其他";}
  var monthNames=["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"];
  return(
    <div style={{background:P.bg,minHeight:"100vh",paddingBottom:80}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",background:P.card,borderBottom:"1px solid "+P.border}}>
        <button type="button" onClick={prevMonth} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:P.ink}}>‹</button>
        <div style={{fontSize:16,fontWeight:700,color:P.ink}}>{year} {monthNames[month]}</div>
        <button type="button" onClick={nextMonth} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:P.ink}}>›</button>
      </div>
      <div style={{padding:"8px 16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:4}}>{["日","一","二","三","四","五","六"].map(function(d){return <div key={d} style={{textAlign:"center",fontSize:11,color:P.muted,padding:"4px 0"}}>{d}</div>;})}</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
          {Array(firstDay).fill(null).map(function(_,i){return <div key={"e"+i}/>;})}
          {Array(daysInMonth).fill(null).map(function(_,i){var d=i+1;var m2=String(month+1);if(m2.length<2)m2="0"+m2;var d2=String(d);if(d2.length<2)d2="0"+d2;var dateStr=year+"-"+m2+"-"+d2;var isToday=dateStr===todayStr,isSel=dateStr===selDate,hasDot=dotDates[dateStr];return(<div key={d} onClick={function(){setSelDate(dateStr);}} style={{textAlign:"center",padding:"6px 0",borderRadius:8,cursor:"pointer",background:isSel?P.btn:isToday?P.block:"transparent",color:isSel?"#fff":isToday?P.btn:P.ink,fontWeight:isToday||isSel?700:400,fontSize:13,position:"relative"}}>{d}{hasDot&&<div style={{position:"absolute",bottom:2,left:"50%",transform:"translateX(-50%)",width:4,height:4,borderRadius:"50%",background:isSel?"rgba(255,255,255,0.8)":P.btn}}/>}</div>);})}
        </div>
      </div>
      <div style={{padding:"0 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{fontSize:14,fontWeight:600,color:P.ink}}>{selDate}</div>

        </div>
        {selRecs.length===0&&<div style={{textAlign:"center",color:P.muted,padding:16,fontSize:13}}>當日無紀錄</div>}
        {selRecs.map(function(r){return(<SwipeRow key={r.id} onEdit={function(){openEdit(r);}} onDelete={function(){handleDelete(r.id);}}><div onClick={function(){setDetailRec(r);}} style={{padding:"12px",background:P.card,borderRadius:12,border:"1px solid "+P.border,cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span style={{fontSize:13,fontWeight:600,color:P.ink}}>{typeLabel(r.type)}</span>
                  {(r.catIds||[]).length>0&&(
                    <div style={{display:"flex",alignItems:"center"}}>
                      {(r.catIds||[]).map(function(cid){var cat=cats.find(function(c){return c.id===cid;});return cat?(cat.photo?<img key={cid} src={cat.photo} title={cat.name} style={{width:22,height:22,borderRadius:"50%",objectFit:"cover",border:"1.5px solid #fff",marginLeft:-4,flexShrink:0}}/>:<div key={cid} title={cat.name} style={{width:22,height:22,borderRadius:"50%",background:"linear-gradient(135deg,#F5DFE0,#EDD5C5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,border:"1.5px solid #fff",marginLeft:-4,flexShrink:0}}>🐱</div>):null;})}
                    </div>
                  )}
                </div>{(r.photos||[]).length>0&&(
                  <div style={{display:"flex",gap:4,marginTop:6}}>
                    {(r.photos||[]).slice(0,3).map(function(p,i){return <img key={i} src={p} style={{width:44,height:44,borderRadius:8,objectFit:"cover",border:"1px solid "+P.border}}/>;}) }
                    {(r.photos||[]).length>3&&<div style={{width:44,height:44,borderRadius:8,background:P.block,border:"1px solid "+P.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:600,color:P.muted}}>+{(r.photos||[]).length-3}</div>}
                  </div>
                )}</div></SwipeRow>);})}
      </div>
      {detailRec&&<RecordDetailPage rec={detailRec} cats={cats} onBack={function(){setDetailRec(null);}} onEdit={function(){openEdit(detailRec);setDetailRec(null);}} onDelete={function(){handleDelete(detailRec.id);setDetailRec(null);}}/>}
      {showForm&&(
        <Modal title={editRec?"編輯紀錄":"新增紀錄"} onClose={function(){setShowForm(false);onModalClose();}} onSave={function(){console.log("save",form);handleSave();}}>
          ...
          <FieldRow label="類型"><select value={form.type} onChange={function(e){setForm(Object.assign({},form,{type:e.target.value}));}} style={{width:"100%",padding:"10px",borderRadius:10,border:"1px solid "+P.border,background:P.input,color:P.ink,fontSize:15}}>{["換貓砂","洗貓砂盆","洗玩具用品","洗澡","其他"].map(function(t){return <option key={t} value={t}>{t}</option>;})}</select></FieldRow>
...
export default function App(){
  ...
  return(
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",position:"relative"}}>
      ...
      {page!=="admin"&&(<div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:P.card,borderTop:"1px solid "+P.border,display:"flex",zIndex:50}}>
        {TABS.map(function(t){var active=page===t.id;return(
          <button type="button" key={t.id} onClick={function(){setPage(t.id);}} style={{flex:1,padding:"10px 4px 12px",background:"none",border:"none",cursor:"pointer",fontSize:11,color:active?P.btn:P.muted,fontWeight:active?700:400,borderTop:active?"2px solid "+P.btn:"2px solid transparent"}}>{t.label}</button>
        );})}
      </div>)}
    </div>
  );
